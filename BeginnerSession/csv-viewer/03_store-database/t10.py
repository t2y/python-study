# -*- coding: utf-8 -*-
"""
contextlib を使って with 文を活用する

* http://docs.python.jp/3.5/library/contextlib.html
"""
...
class db(contextlib.ContextDecorator):
    ...
    def __exit__(self, exc_type, exc, exc_tb):
        log.debug('__exit__ called')
        try:
            if exc is None:
                self.conn.commit()
                log.debug('committed database')
        finally:
            self.conn.close()
...
def store_csv_data(reader):
    with db() as conn:
        ...
        # db handles open/commit/close database as context manager
